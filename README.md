...
[`chai-toronto/SpeechDx`](https://github.com/chai-toronto/SpeechDx), which is the
source of truth for the scores (`leaderboard.csv` on `main`):
...
Then copy that `docs/data.js` here and push. The board mirrors
`leaderboard.csv` on `chai-toronto/SpeechDx@main`
(`scripts/leaderboard.py --no-merge --mrr --drop ...`): the 12 published external
encoders, re-ranked among themselves, with the from-scratch and LLM columns
dropped.
